...
def get_all_auth_salida_pag(db: Session, skip:int = 0, limit = 10, search: str = ""):
    """
    ...
    try: 
        ...
        if search:
            where_clause = """
                WHERE CAST(a_s.id_autorizacion AS CHAR) LIKE :search
                   OR u.nombre_usuario LIKE :search
                   OR e.serial LIKE :search
                   OR t.nombre_tipo LIKE :search
                   OR c.nombre_categoria LIKE :search
                   OR a_s.destino LIKE :search
                   OR a_s.motivo LIKE :search
                   OR (CASE WHEN a_s.estado THEN 'autorizado' ELSE 'pendiente' END) LIKE :search
            """
            query_params["search"] = f"%{search}%"

        count_query = text(f"""
            SELECT COUNT(a_s.id_autorizacion) AS total
            FROM autorizacion_salida as a_s
            INNER JOIN usuarios as u ON u.id_usuario = a_s.usuario_id_autoriza
            INNER JOIN equipos_sede_inv as e ON e.id_equipo_sede = a_s.equipo_id
            INNER JOIN tipo_movimientos as t ON t.id_tipo = a_s.tipo_id
        ...
        data_query = text(f"""
            ...
            LIMIT :limit OFFSET :skip
        """)
        auth_salida_list = db.execute(data_query, query_params).mappings().all()
