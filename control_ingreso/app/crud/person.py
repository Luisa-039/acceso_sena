...
def get_all_persons_pag(db: Session, skip:int = 0, limit = 10, search: str = ""):
    """
    ...
    try: 
        ...
        if search:
            where_clause = """
                WHERE tipo_persona LIKE :search
                   OR tipo_documento LIKE :search
                   OR documento LIKE :search
                   OR nombre_completo LIKE :search
                   OR CAST(id_persona AS CHAR) LIKE :search
                   OR (CASE WHEN estado THEN 'activo' ELSE 'inactivo' END) LIKE :search
            """
            query_params["search"] = f"%{search}%"

        count_query = text(f"""
            SELECT COUNT(id_persona) AS total
            FROM personas
            {where_clause}
        """)
        ...
        """)
        persons_list = db.execute(data_query, query_params).mappings().all()
