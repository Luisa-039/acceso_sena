...
def get_all_equipements_pag(db: Session, skip:int = 0, limit = 10, search: str = ""):
    """
    ...
    try: 
        ...
        if search:
            where_clause = """
                WHERE eq.serial LIKE :search
                   OR eq.codigo_barras_inv LIKE :search
                   OR eq.descripcion LIKE :search
                   OR eq.marca_modelo LIKE :search
                   OR p.nombre_completo LIKE :search
                   OR c.nombre_categoria LIKE :search
                   OR CAST(eq.id_equipo AS CHAR) LIKE :search
                   OR (CASE WHEN eq.estado THEN 'activo' ELSE 'inactivo' END) LIKE :search
            """
            query_params["search"] = f"%{search}%"

        count_query = text(f"""
            SELECT COUNT(eq.id_equipo) AS total
            FROM equipos_externos as eq
            INNER JOIN personas as p ON eq.persona_id = p.id_persona
            INNER JOIN categorias as c ON eq.categoria_id = c.id_categoria
            {where_clause}
        ...
        #2 Consultar equipos
        data_query = text(f"""
            SELECT eq.id_equipo, eq.serial, eq.codigo_barras_inv, eq.descripcion,
                   eq.categoria_id, eq.foto_path, eq.marca_modelo, eq.persona_id, eq.fecha_registro,
                   p.nombre_completo as nombre_completo, eq.estado, c.nombre_categoria
            FROM equipos_externos as eq
            INNER JOIN personas as p ON eq.persona_id = p.id_persona
