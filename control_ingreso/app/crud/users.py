...
def get_all_users_pag(db: Session, skip:int = 0, limit = 10, search: str = ""):
    """
    ...
    try: 
        ...
        where_clause = "WHERE u.rol_id != 1"
        if search:
            where_clause += """
                AND (
                    u.nombre_usuario LIKE :search
                    OR u.documento LIKE :search
                    OR u.email LIKE :search
                    OR u.telefono LIKE :search
                    OR s.nombre LIKE :search
                    OR r.nombre LIKE :search
                    OR CAST(u.id_usuario AS CHAR) LIKE :search
                    OR (CASE WHEN u.estado THEN 'activo' ELSE 'inactivo' END) LIKE :search
                )
            """
        ...
        data_query = text(f"""
            SELECT u.id_usuario, u.rol_id, u.nombre_usuario, u.documento, u.email,
                   u.telefono, u.estado, u.sede_id, s.nombre, r.nombre AS nombre_rol
            FROM usuarios as u
            INNER JOIN sedes as s ON u.sede_id = s.id_sede
            INNER JOIN roles as r ON u.rol_id = r.id_rol
        ...
        """)
        users_list = db.execute(data_query, query_params).mappings().all()
